...
Create `_data/metrics/develop/2026-02-04.json`:
```json
{
...
The dashboard supports viewing metrics from multiple branches (e.g., `main`, `develop`).

### How It Works

1. **Storage:** Metrics are stored in branch-specific subdirectories:
   - `_data/metrics/develop/2026-02-07.json`
   - `_data/metrics/main/2026-02-07.json`

2. **Branch Selector:** The dashboard includes a dropdown to switch between branches

3. **Default Branch:** `main` is shown by default if it exists, otherwise `develop`

4. **Legacy Support:** Old metrics stored in `_data/metrics/*.json` (without branch subdirectory) are automatically migrated to `develop`

### Branch Order

Branches appear in the selector in this order:
1. `main` (if exists)
2. `develop`
...
├── _data/
│   └── metrics/
│       ├── develop/              # Branch-specific directory
│       │   ├── 2026-02-04.json
│       │   ├── 2026-02-05.json
│       │   └── latest.json
│       └── main/                 # Branch-specific directory
│           ├── 2026-02-07.json
│           └── latest.json
