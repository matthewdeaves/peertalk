...
CORE_SRCS = src/core/pt_version.c src/core/pt_compat.c src/core/pt_init.c src/core/protocol.c src/core/peer.c src/core/peer_multi.c src/core/queue.c src/core/send.c src/core/direct_buffer.c src/core/stream.c src/core/buffer_pool.c
...
$(BIN_DIR)/test_compact_header: tests/test_compact_header.c $(LIBPEERTALK) $(LIBPTLOG) | $(BIN_DIR)
	$(CC) $(CFLAGS) -o $@ $< -L$(LIB_DIR) -lpeertalk -lptlog $(LDFLAGS)
...
test-compact-header: $(BIN_DIR)/test_compact_header
	@echo "Running compact header tests..."
	@$(BIN_DIR)/test_compact_header
...
.PHONY: all test test-local test-log test-compat test-foundation test-protocol test-peer \
        ...
        test-queue-threads test-unity test-direct-buffer test-two-tier-queue test-compact-header \
