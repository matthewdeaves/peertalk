                                       ...
                                       unsigned long now)
{
    ...
    /* Process deferred log events from completion routine */
    if (hot->log_events & PT_LOG_EVT_CONNECT_COMPLETE) {
        if (hot->log_events & PT_LOG_EVT_ERROR) {
            PT_LOG_DEBUG(ctx->log, PT_LOG_CAT_CONNECT,
                "Connect completion: err=%d", (int)hot->log_error_code);
        }
        hot->log_events &= ~(PT_LOG_EVT_CONNECT_COMPLETE | PT_LOG_EVT_ERROR);
    }
...
/**
 * Poll a connected stream for incoming data.
 */
                                    ...
                                    unsigned long now)
{
    ...
    if (hot->log_events & PT_LOG_EVT_CLOSE_COMPLETE) {
        PT_LOG_DEBUG(ctx->log, PT_LOG_CAT_CONNECT,
            "Close completion: result=%d", (int)hot->log_error_code);
        hot->log_events &= ~(PT_LOG_EVT_CLOSE_COMPLETE | PT_LOG_EVT_ERROR);
    }
...
int pt_mactcp_poll(struct pt_context *ctx)
{
    ...
    for (i = 0; i < (int)ctx->max_peers; i++) {
        ...
        if ((long)(now - peer->hot.last_seen) > (long)PT_PEER_TIMEOUT_TICKS) {
            PT_LOG_DEBUG(ctx->log, PT_LOG_CAT_CONNECT,
                "Peer %u timed out", (unsigned)peer->cold.info.id);
