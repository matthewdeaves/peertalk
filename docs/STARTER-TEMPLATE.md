...
Use this branch to see how Claude Code skills, agents, and hooks work together on a real project.

**Target audience:** Developers who know Claude Code and want to study domain-specific customization patterns.
...
**No SDK code** - you implement from the plans.
...
- **`classic-mac-hardware`** - FTP access to Classic Mac test machines. Provides resources (logs, binary info) and tools (deploy, fetch). Uses RumpusFTP (plain FTP, passive mode). Follows Anthropic's code execution pattern for 98%+ token savings. See `SETUP.md` for configuration.
